...
def get_exposed_modules(conf_path):
    ...
    Returns:
      visible: every module name the package exposes (owned + re-exports)
      owned:   modules the package actually defines (no re-exports)

    The split matters because GHC treats re-exports as pointing at the same
    module identity as the original — a re-export and its origin aren't a
    real conflict at `import` time. Callers that resolve conflicts can
    ignore re-exports and let the owner win automatically.
    """
    field = _read_field(conf_path, "exposed-modules")
    tokens = _tokenize(field)
    return _classify(tokens)


def _read_field(conf_path, field_name):
    """Return the value of a multi-line .conf field as one whitespace-joined string."""
    header = field_name + ":"
    chunks = []
    in_field = False
...
def _tokenize(field):
    """Split a .conf list field on whitespace and/or commas.

    Either is a valid entry separator (different packages' confs use
    different styles; ghc-pkg doesn't normalize between them).
    """
    return [t for t in re.split(r"[\s,]+", field) if t]


def _is_reexport_at(tokens, i):
    """True if tokens[i:i+3] matches `Mod from <pkgid>:<OrigName>`."""
    return (
        i + 2 < len(tokens)
        and tokens[i + 1] == "from"
        and ":" in tokens[i + 2]
...
def _classify(tokens):
    """Walk an exposed-modules token stream, returning (visible, owned) sets.

    Each entry is one of:
      - `Mod`                          — owned by this package
      - `Mod from <pkgid>:<OrigName>`  — re-exported from another package

    Both contribute to `visible`; only the bare-`Mod` shape contributes to
    `owned`. The three-token re-export form is consumed atomically so we
    don't pick up the `pkgid:OrigName` token as a separate module name.
    """
    visible = set()
    owned = set()
    i = 0
